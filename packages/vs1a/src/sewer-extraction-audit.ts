...
export interface SewerExtractionAuditCandidateSnapshot {
  candidate_id: string;
  project_instance_id: string;
  source_document_id: string;
  source_sheet_id: string;
...
export interface SewerExtractionAuditRecord {
  ...
  trace_refs: string[];
  candidate_snapshot: SewerExtractionAuditCandidateSnapshot;
}
...
const STRONG_NON_SEWER_TOKENS = ["storm", "water", "domestic water", "fire line", "gas", "electrical", "power", "telecom", "irrigation"];
const STRONG_SEWER_PATTERNS = [/\bsanitary\b/u, /\bsewer\b/u, /\bss[-\s]?[a-z0-9]*\b/u, /\bsan\b/u];
...
function classifyCandidate(candidate: SewerExtractionAuditCandidateInput, input: SewerExtractionAuditInput, sheetIds: Set<string>): SewerExtractionAuditRecord {
  ...
  if (traceRefs.length === 0) throw new Error("audit_trace_refs_required");
  return {
...
function assertCandidate(candidate: SewerExtractionAuditCandidateInput, input: SewerExtractionAuditInput, sheetIds: Set<string>): void {
  if (candidate.project_instance_id !== input.project_instance_id) throw new Error("audit_project_instance_id_mismatch");
  if (candidate.source_document_id !== input.source_document_id) throw new Error("audit_source_document_id_mismatch");
  if (!sheetIds.has(candidate.source_sheet_id)) throw new Error("audit_source_sheet_not_found");
}
...
function countByDecision(records: SewerExtractionAuditRecord[]): SewerExtractionAuditDecisionCounts {
  return records.reduce<SewerExtractionAuditDecisionCounts>((acc, record) => {
    acc[record.decision] += 1;
    return acc;
  }, { found_sewer_run: 0, uncertain_sewer_candidate: 0, excluded_non_sewer_item: 0, unresolved_plan_question: 0 });
}
